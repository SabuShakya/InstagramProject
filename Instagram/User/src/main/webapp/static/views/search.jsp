    ...
    <div class="large-padded-row">
        ...
        <div class="padded-row">
            ...
            <div angucomplete
                 ...
                 url="http://localhost:8080/anguSearch/{{search.profileOfUser}}/"
                 titlefield="username"
                 input-class="form-control form-control-small">
            </div>
            <br><br>
            <button type="button" class="btn-default" ng-disabled="search.clicked" ng-click="search.search()">Search</button>
        </div>
    </div>
</div>
...
<div ng-show="search.showList">
    <div ng-repeat="user in search.finalList">
        ...
        <h5>{{user.fullname}}</h5>
        <div ng-hide="user.blockStatus">
            <div ng-hide="user.showResultButtons">
                <button type="button" class="btn-default" ng-disabled="search.clicked" ng-click="search.followUser(user)">Follow</button>
            </div>

            <div ng-show="user.showResultButtons">
                <button type="button" class="btn-default" ng-disabled="search.clicked" ng-click="search.unfollowUser(user)">Un-Follow</button>
            </div>
        </div>
        <div ng-hide="user.blockStatus">
            <button type="button" class="btn-default" ng-disabled="search.clicked" ng-click="search.blockUser(user)">Block</button>
        </div>

        <div ng-show="user.blockStatus">
            <button type="button" class="btn-default" ng-disabled="search.clicked" ng-click="search.unblockUser(user)">Un-Block</button>
        </div>
    </div>
</div>
